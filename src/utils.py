...
def set_optim(opt, model_list, freeze_part=[],no_decay=[],large_lr=[], accumulation_step=None):
    named_parameters = []
    param_name = []
    ...
    # group_names = ["Weight Decay Group", "No Decay Group", "Large LR Group"]
    # for i, group in enumerate(parameters):
    #     print(f"=== {group_names[i]} ===")
    #     print(f"Learning Rate: {group['lr']}, Weight Decay: {group['weight_decay']}")
    ...
    if opt.scheduler == 'fixed':
        ...
        print(int(opt.warmup_steps / accumulation_step), int(opt.total_steps / accumulation_step), opt.lr)
...
def normalize_sparse_matrix(matrix):
    """
    Normalize a sparse matrix in COO format such that the sum of each row is 1.
    """
    rowsum = matrix.to_dense().sum(dim=1)

    # Normalize using broadcasting
    rowsum_inv = rowsum.pow(-1).flatten()
    rowsum_inv[torch.isinf(rowsum_inv)] = 0.
    
    # Create diagonal matrix
    diagonal_indices = torch.arange(0, len(rowsum_inv)).to(matrix.device)
    diagonal = torch.sparse_coo_tensor(
        indices=torch.vstack((diagonal_indices, diagonal_indices)),
        values=rowsum_inv,
        size=(len(rowsum_inv), len(rowsum_inv))
    )
        # Convert diagonal matrix to dense format
    diagonal_dense = diagonal.to_dense()

    # Normalize using dense matrix multiplication
    normalized_matrix_dense = torch.mm(diagonal_dense, matrix.to_dense())

    # (Optional) Convert result back to sparse format
    normalized_matrix_sparse = normalized_matrix_dense.to_sparse()


    return normalized_matrix_sparse
...
def get_adjr(ent_size, triples, norm=False):
    ...
    if norm:
        ...
        # pdb.set_trace()
        return sparse_mx_to_torch_sparse_tensor(normalize_adj(adj)),neighbours
    else:
...
def csls_sim(sim_mat, k, m_csls=3):
    """
